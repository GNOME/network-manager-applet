...
qr_code_draw (GtkWidget *widget, cairo_t *cr, gpointer user_data)
{
	...
	cairo_scale (cr,
		(float)gtk_widget_get_allocated_width (widget) / (size + 2),
		(float)gtk_widget_get_allocated_height (widget) / (size + 2));
...
draw_page (GtkPrintOperation *operation, GtkPrintContext *context, gint page_nr, gpointer user_data)
{
	...
	gdouble spacing_x = (width - (count_x * CARD_WIDTH_PT)) / (count_x + 1);
	gdouble spacing_y = (height - (count_y * CARD_HEIGHT_PT)) / (count_y + 1);
...
static gboolean
link_activated (GtkLabel *label, gchar *uri, gpointer user_data)
{
	NMABarCodeWidget *self = NMA_BAR_CODE_WIDGET (user_data);
...
nma_bar_code_widget_class_init (NMABarCodeWidgetClass *klass)
{
	...
	gtk_widget_class_bind_template_callback (widget_class, qr_code_draw);
